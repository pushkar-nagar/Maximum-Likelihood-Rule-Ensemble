...
from Cut import Cut
class RuleBuilder():
    def __init__(self, subsample = None,  nrules = None, traindf = None, testdf = None, shrink = .5, useLineSearch = False, method = 0, prechosenK = False):
        ...
        self.invertedList = np.zeros((self.D,self.N), dtype = np.int64)
        self.attributeNames = list(self.traindf.columns.values)
        self.attributeNames.remove("label")
        for attidx,attribute in enumerate(self.attributeNames):
            self.traindf = self.traindf.sort_values(by=[attribute], ascending=True)
            #import pdb
            #pdb.set_trace()
            self.invertedList[attidx,:] = np.int64(self.traindf.index)

        self.traindf.sort_index(inplace=True)
    ...
    def initializeForRule(self, func, coveredInstances):
        ...
        for i in range(self.N):
            if coveredInstances[i] >= 0:
                ...
                if(self.prechosenK):
                    #import pdb; pdb.set_trace()
                    self.gradients[int(self.traindf.label.iloc[i])] += self.traindf.iloc[i].weight
                
        if(self.prechosenK):
            self.maxk = 0
            if(self.method == 0):
                self.maxk = np.argmax(self.gradients)
            else:
                for k in range(self.K):
                    if (self.gradients[k] / math.sqrt(self.hessians[k])) > (self.gradients[self.maxk] / math.sqrt(self.hessians[self.maxk])):
                        self.maxk = k
        return self.maxk
    ...
    def computeDecision(self, coveredInstances):
        ...
        if(self.prechosenK):
            ...
            for i in range(self.N):
                if coveredInstances[i] >= 0:
                    if self.traindf.label.iloc[i] == self.maxk:
            ...
            for i in range(self.N):
                if coveredInstances[i] >= 0:
                    for k in range(self.K):
                        if self.traindf.label.iloc[i] == k:
    ...
    def getLineSearchDecision(self, func, coveredInstances):
        ...
        if gradient >= 0:
            decision = np.zeros(self.K, dtype = np.float)
            decision.fill(-self.lineSearchMax / self.K)
    ...
    def createDefaultRule1(self):
        ...
        for i in range(len(self.N)):
            priors[int(self.traindf.label.iloc[i])] += 1
        emptyClasses = 0
    ...
    def createRule(self, func, coveredInstances):
        ...
        rule = Rule()
        bestCut = Cut(K = self.K, N = self.N)
        bestCut.empiricalRisk = 0
        creating = True
        while(creating):
            print("while loop running for elementry expression creation")
            bestAttribute = -1
            cut = Cut(K = self.K, N = self.N)
            for j in range(self.D):
                ...
                if getCut.empiricalRisk < (bestCut.empiricalRisk - self.EPSILON):
                    ...
                    bestAttribute = j
            if bestAttribute == -1 or bestCut.exists == False:
                creating = False
            else:
...
if __name__ == "__main__":
    print("The problem is , we think we have time ")